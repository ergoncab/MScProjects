...
# Parallel 
clusterExport(clust, list("k_means","computers"))
wss<-parSapply(clust,1:n, function(x) k_means(x+1))
...
# Obtain coords of the first two dimensions
pca<-PCA(computers, scale.unit = FALSE, ncp = 2, graph=FALSE)
coords <-pca$ind$coord

# Sockets execution time
last<-Sys.time()
print("Sockets execution time:")
print(last-initial)
